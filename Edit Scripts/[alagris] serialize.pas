...
begin
  ...
  code := code + '    end;'#13#10; 
  code := code + 'end;'#13#10;
  code := code + 'function newLVLI(destFile: IwbFile; eID, lvld, useAll, calcEach, calcLvl:string): IwbElement;'#13#10;
  code := code + 'begin'#13#10;
  code := code + '    if Assigned(MainRecordByEditorID(GroupBySignature(destFile,''LVLI''), eID)) then begin'#13#10;
  code := code + '        exit;'#13#10;  
  code := code + '    end;'#13#10;  
  code := code + '    Result := Add(GroupBySignature(destFile, ''LVLI''), ''LVLI'', true);'#13#10;  
  code := code + '    SetEditorID(Result, eID);'#13#10;  
  code := code + '    SetElementEditValues(Result, ''LVLD'', lvld);'#13#10;    
  code := code + '    SetElementEditValues(Result, ''LVLF\Calculate from all levels <= player''''s level'', calcLvl);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLF\Calculate for each item in count'', calcEach);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLF\Use All'', useAll);'#13#10;
  code := code + '    Result := Add(Result, ''Leveled List Entries'', true);'#13#10;
  code := code + 'end;'#13#10;
  code := code + 'function addToLVLI(destFile: IwbFile; e: IwbElement; filename, signat, ref, count, level:string): IwbElement;'#13#10;
  code := code + 'begin'#13#10;
  code := code + '    AddMasterIfMissing(destFile, filename);'#13#10;
  code := code + '    Result := Add(e, ''Leveled List Entry'', true);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Reference'', Name(MainRecordByEditorID(GroupBySignature(FileByName(filename), signat), ref)));'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Count'', count);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Level'', level);'#13#10;
  code := code + 'end;'#13#10;
  code := code + 'function addToLVLI_(destFile: IwbFile; e: IwbElement; signat, ref, count, level:string): IwbElement;'#13#10;
  code := code + 'begin'#13#10;
  code := code + '    Result := Add(e, ''Leveled List Entry'', true);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Reference'', Name(MainRecordByEditorID(GroupBySignature(destFile, signat), ref)));'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Count'', count);'#13#10;
  code := code + '    SetElementEditValues(Result, ''LVLO\Level'', level);'#13#10;
  code := code + 'end;'#13#10;
